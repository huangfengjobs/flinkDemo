...
/**
  ...
  */
object Sensor_transform_connect {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val warming: DataStream[(String, Double)] = high.map(data => (data.id, data.temperature))
    val connected: ConnectedStreams[(String, Double), SensorRecord] = warming.connect(low)
    val coMapStream: DataStream[(String, Double, String)] = connected.map(
      warmingData => (warmingData._1, warmingData._2, "warming"),
